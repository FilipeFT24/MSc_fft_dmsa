classdef B_1D
    methods(Static)
        ...
        function [msh,pde] = WrapUp_B_1D(inp,msh)
            ...
            [msh,pde] = B_1D.SetUp(msh,v,g,"exp",bnd,p_adapt,n,ee,t1,t2);
        end
        ...
        % >> 2. -----------------------------------------------------------
        function [msh,pde] = SetUp(msh,v,g,ft,bnd,p_adapt,n,ee,tv,tg)
            % >> Select...
            switch ee
                case false
                    % >> Initialize.
                    ...
                    switch p_adapt
                        case false
                            %  > ...solve PDE.
                            [s,x,ea] = B_2_1D.Update_stl(msh,s,stl.p,stl.s,stl.t,A,B,pde.a,bnd,v,g);
                            [e,x]    = B_2_1D.Update_pde(msh,pde.a,s,x,ea,v,g);
                        case true
                            %  > ...solve PDE while...
                            i = 0;
                            while i < n
                                [s,x] = B_2_1D.Update_stl(msh,s,stl.p,stl.s,stl.t,A,B,pde.sn,bnd,v,g);
                                [e,x] = B_2_1D.Update_pde(msh,x,pde.sn,s,v,g);
