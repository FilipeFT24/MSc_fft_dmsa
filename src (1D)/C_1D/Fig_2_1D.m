classdef Fig_2_1D
    methods (Static)         
        ...
        function [] = SubPlot_2(msh,pde,Xvi,Xvf,C1,L)
            hold on;
            P{1} = plot(msh.f.Xv,pde.e.f.f(:,1)             ,'-o','Color',C1(1,:),'LineWidth',1.5,'MarkerFaceColor','w','MarkerSize',3.5);
            P{2} = plot(msh.f.Xv,pde.e.f.f(:,2)             ,'-o','Color',C1(2,:),'LineWidth',1.5,'MarkerFaceColor','w','MarkerSize',3.5);
            P{3} = plot(msh.c.Xc,pde.e.c.c(:,1)             ,'-s','Color',C1(3,:),'LineWidth',1.5,'MarkerFaceColor','w','MarkerSize',3.5);
            P{4} = line([Xvi,Xvf],[pde.e.f.n(1,1),pde.e.f.n(1,1)],'Color',C1(1,:),'Linewidth',1.0,'Linestyle','--');
            P{5} = line([Xvi,Xvf],[pde.e.f.n(1,2),pde.e.f.n(1,2)],'Color',C1(2,:),'Linewidth',1.0,'Linestyle','-.');
            P{6} = line([Xvi,Xvf],[pde.e.c.n(1,1),pde.e.c.n(1,1)],'Color',C1(3,:),'Linewidth',1.0,'Linestyle',':' );
            %  > Legend.
            legend([P{:}],[L{:}],...
                'Interpreter','latex','Location','Northeast','FontSize',10,'NumColumns',2);            
            %  > Axis.
        ...
        function [L] = Set_Labels_2()
            ...
            L{5} = "$|\!|e_{f}^{\nabla\phi}|\!|_{1}$";
            L{6} = "$|\!|e_{c}^{\phantom{\nabla}\phi}|\!|_{1}$";
            L{7} = "$\textrm{Error magnitude}\left(|e^{\phi}|\right)$";
        end
    end
end      