    ...
    methods (Static)
        ...
        function [] = Plot(plot,inp,msh,obj)
            ...
            if ~exp
                ...
                %  > #2.
                if plot(2) && inp.t_terms.allow
                    figure; set(gcf,'Units','pixels','Position',fig.Position);
                    subplot(1,2,1);
                    x.a = 1;
                    x.b = 1;
                    Fig_V1_1_1D.Plot_2_1(inp,msh.f.Xv,obj.e,x,fig,0);
                    subplot(1,2,2);
                    x.a = 2;
                    x.b = 1;
                    Fig_V1_1_1D.Plot_2_1(inp,msh.f.Xv,obj.e,x,fig,0);
        ...
        % >> 3.1. ---------------------------------------------------------
        %  > 3.1.1. -------------------------------------------------------
        function [] = Plot_2_1(inp,Xv,obj_e,x,fig,zoom)
            %  > Auxiliary variables.
            fid = "V1_1_1D (3)";
            j   = x.a;
            n   = x.b;
            sz  = size(obj_e.a{n}.m.f_abs{j},2);
           
            %  > Select variables.
            M1  = ["-v",repelem(":o",sz)];
            L1  = Fig_V1_1_1D.Set_Legend_3(j,1:sz);
            V1  = [obj_e.a{n}.t.f_abs(:,j),obj_e.a{n}.m.f_abs{j}]; 
            %  > Plot variables.
            [L1,P1,Y1] = Fig_Tools_1D.Var_1(fig,M1,L1,Xv,V1);
            
            %  > Axis/legend,etc.
            Fig_Tools_1D.Set_Plot_2(fig,L1,P1,1,Xv,Y1,[-1,1],1);
            %  > Export(?)/Zoom(?).
            Fig_Tools_1D.Exp_Or_Zoom(fig,zoom,fid);
        end
        %  > 3.1.2. -------------------------------------------------------
        function [L] = Set_Legend_3(n,p)
            S     = Fig_Tools_1D.Set_str_2(n);
            L{1}  = join(["$|\bar{\tau}_{f^{\left(a\right)}}^{",S,"}|$"]);
            for i = 1:length(p)
                L{i+1} = join(["$|\bar{\tau}_{f^{\left(",num2str(i-n),"\right)}}^{",S,"}|$"]);
            end
        end
    end
end