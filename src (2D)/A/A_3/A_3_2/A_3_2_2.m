classdef A_3_2_2
    methods (Static)
        ...
        %  > 1.2.2. Extension #1.
        %  > 1.2.3. Extension #2. (NOT USED)
        % >> --------------------------------------------------------------
        function [msh] = Extend_Stencil(msh,bnd_cc,bnd_ff,bnd_fc,nt,np_x,np_y,et)
            ...
            for i = 1:msh.f.NF
                ...
                % >> Check...
                if ceil(par.nx(i)) >= np_x && ceil(par.ny(i)) >= np_y
                    if ~et
                        continue;
                    end
                else
                    while (ceil(par.nx(i)) < np_x || ceil(par.ny(i)) < np_y) && Continue
                        %% > x-direction.
                        Continue_X = false;
